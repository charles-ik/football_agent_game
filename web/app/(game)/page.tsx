...
export default async function OfficePage() {
 ...
 return <div className="space-y-5">
   ...
   <section className="grid grid-cols-2 gap-3 lg:grid-cols-4"><StatTile label="Agency balance" value={<Money value={game.agency.cash}/>}/><StatTile label="Weekly operations" value={<Money value={game.weekly_net} signed/>} tone={game.weekly_net.amount<0?"warn":"good"}/><StatTile label="Careers in your hands" value={`${clients.length} / ${game.counts.client_cap}`}/><StatTile label="Your standing" value={game.agency.reputation_label}/></section>
   <div className="grid gap-4 lg:grid-cols-2">
    <PanelSection title="Your next chapter" actions={<Flag size={16} className="text-accent"/>}><p className="office-title text-2xl capitalize">{objective.id}</p><p className="t-note mt-1">Season {objective.season} objective · {objective.completed?"Completed":"In progress"}</p><progress className="mt-3 h-2 w-full accent-[var(--color-accent)]" value={objective.progress} max={objective.target} aria-label="Season objective progress"/><div className="mt-2 flex justify-between text-sm"><span className="num">{objective.progress} / {objective.target}</span><Link href="/season-review" className="text-accent">View your story ↗</Link></div></PanelSection>
