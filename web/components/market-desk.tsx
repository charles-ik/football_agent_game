...
const button = "rounded border border-line px-3 py-2 text-xs hover:bg-panel-2 disabled:opacity-40";
const field = "rounded border border-line bg-panel px-3 py-2 text-sm";
export function MarketDesk({ market, clients, revision, initialClub }: {
    ...
    <Panel className="flex flex-wrap items-end justify-between gap-4 p-4"><label className="grid gap-2 text-xs text-dim">Representing<select className={field} value={pid} onChange={e => { setPid(Number(e.target.value)); setNotice(""); setPage(0); }}>{clients.map(c => <option key={c.player.id} value={c.player.id}>{c.player.name} · {c.player.position}</option>)}</select></label><div className="text-right"><Badge tone={market.window_open ? "good" : "warn"}>{market.window_open ? "Transfer window open" : "Transfer window closed"}</Badge><p className="t-note mt-2">{client.club_name} · {client.wage?.text ?? "No club wage"} per week</p></div></Panel>
