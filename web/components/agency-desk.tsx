...
import { SeasonObjective, objectiveLabels } from "@/components/season-objective";
...
export function AgencyDesk({management:m, clients, clubs, game, hq}:{management:ManagementDTO; clients:ClientRow[]; clubs:PortfolioClub[]; game:GameState; hq:HQState}) {
  ...
  return <div className="space-y-5">
    ...
    <fieldset disabled={pending} className="min-w-0 space-y-5 disabled:opacity-70">
      ...
      <PanelSection title="Your next chapter" actions={<Link href="/season-review" className="text-xs text-accent underline">Agency record</Link>}>
        <SeasonObjective objective={m.objective}/>
        <div className="mt-5 grid gap-2 sm:grid-cols-3">{m.objective_options.filter(option=>option.id!==m.objective.id).map(option=><button key={option.id} className={`${buttonClass.secondary} text-left`} disabled={game.calendar.season_week!==1 || m.objective.completed} onClick={()=>void run("objective",{objective:option.id})}><span className="capitalize">{option.id}</span><span className="mt-1 block text-xs font-normal">{option.target} {objectiveLabels[option.id]?.unit} · +{option.reward} reputation</span></button>)}</div>
        <p className="t-note mt-3">Choose your ambition in the first week of each season.</p>
      </PanelSection>
